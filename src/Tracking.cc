...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    double timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
    mpSystem->mvImageDealCost.push_back(timecost);
...
void Tracking::Track()
{
    ...
    if(mState==NOT_INITIALIZED)
    {
        ...
        mpSystem->mvInitialCost.push_back(timeCost);
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            std::chrono::steady_clock::time_point t_End_Track = std::chrono::steady_clock::now();
            double timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
            mpSystem->mvTrackCost.push_back(timecost);
        }
        else
        {
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    {
        if (mSensor == System::IMU_MONOCULAR && (mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=mvKfPara[0])
            return true;
    ...
    bNeedToInsertClose = (nTrackedClose<mvKfPara[1]) && (nNonTrackedClose>mvKfPara[2]);

    // Thresholds
    float thRefRatio = mvKfPara[3];
    if(nKFs<2)
        thRefRatio = 0.4f;
    ...
    if(mpCamera2) thRefRatio = mvKfPara[3];
    ...
    const bool c1c = mSensor!=System::MONOCULAR && mSensor!=System::IMU_MONOCULAR && mSensor!=System::IMU_STEREO && mSensor!=System::IMU_RGBD && (mnMatchesInliers<nRefMatches*mvKfPara[4] || bNeedToInsertClose) ;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    const bool c2 = (((mnMatchesInliers<nRefMatches*thRefRatio || bNeedToInsertClose)) && mnMatchesInliers>mvKfPara[5]);
    ...
    if(mpLastKeyFrame)
    {
        if (mSensor==System::IMU_MONOCULAR)
        {
            if ((mCurrentFrame.mTimeStamp-mpLastKeyFrame->mTimeStamp)>=mvKfPara[6])
                c3 = true;
        }
        else if (mSensor==System::IMU_STEREO || mSensor == System::IMU_RGBD)
        {
    ...
    bool c4 = false;
    if ((((mnMatchesInliers<mvKfPara[7]) && (mnMatchesInliers>mvKfPara[8])) || mState==RECENTLY_LOST) && (mSensor == System::IMU_MONOCULAR)) // MODIFICATION_2, originally ((((mnMatchesInliers<75) && (mnMatchesInliers>15)) || mState==RECENTLY_LOST) && ((mSensor == System::IMU_MONOCULAR)))
        c4=true;
    else
        c4=false;
