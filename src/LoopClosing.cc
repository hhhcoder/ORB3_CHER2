...
LoopClosing::LoopClosing(System* pSys, Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC):
    mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas),
    mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mpMatchedKF(NULL), mLastLoopKFid(0), mbRunningGBA(false), mbFinishedGBA(true),
    mbStopGBA(false), mpThreadGBA(NULL), mbFixScale(bFixScale), mnFullBAIdx(0), mnLoopNumCoincidences(0), mnMergeNumCoincidences(0),
    mbLoopDetected(false), mbMergeDetected(false), mnLoopNumNotFound(0), mnMergeNumNotFound(0), mbActiveLC(bActiveLC), mpSystem(pSys)
{
...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            mpSystem->mvLoopDetectCost.push_back(timecost);
#ifdef REGISTER_TIMES
            ...
            if(bFindedRegion)
            {
                if(mbMergeDetected)
                {
                    ...
                    else
                    {
                        ...
                        std::chrono::steady_clock::time_point t_End_Track = std::chrono::steady_clock::now();
                        double timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
                        mpSystem->mvLoopMergeCost.push_back(timecost);
#ifdef REGISTER_TIMES
                ...
                if(mbLoopDetected)
                {
                    ...
                    if (bGoodLoop) {
                        ...
                        std::chrono::steady_clock::time_point t_End_Track = std::chrono::steady_clock::now();
                        double timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
                        mpSystem->mvEssentialGraphOptCost.push_back(timecost);
#ifdef REGISTER_TIMES
