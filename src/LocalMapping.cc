...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            mpSystem->mvLocalMapInsertKfCost.push_back(timecost);
#ifdef REGISTER_TIMES
            ...
            t_Start_Track = std::chrono::steady_clock::now();
            MapPointCulling();
            t_End_Track = std::chrono::steady_clock::now();
            timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
            mpSystem->mvLocalMapMergeMapPointCost.push_back(timecost);
#ifdef REGISTER_TIMES
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())
                    {
                        ...
                        t_End_Track = std::chrono::steady_clock::now();
                        timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
                        mpSystem->mvLocalMapIMU2ndOptCost.push_back(timecost);
                        b_doneLBA = true;
                    }
                    else
                    {
                        ...
                        t_End_Track = std::chrono::steady_clock::now();
                        timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
                        mpSystem->mvLocalMapIMU3rdOptCost.push_back(timecost);
                        b_doneLBA = true;
                    }
                ...
                t_End_Track = std::chrono::steady_clock::now();
                timecost = std::chrono::duration_cast<std::chrono::duration<double,std::milli> >(t_End_Track - t_Start_Track).count();
                mpSystem->mvLocalMapMergeKfCost.push_back(timecost);  
#ifdef REGISTER_TIMES
